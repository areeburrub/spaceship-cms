...
// Functions for Refresh Tokens

export async function encryptRefreshToken(payload: RefreshTokenPayload): Promise<string> {
    return new SignJWT(payload)
        .setProtectedHeader({ alg: 'HS256' })
        .setIssuedAt()
...
export async function decryptRefreshToken(token: string | undefined = ''): Promise<RefreshTokenPayload | null> {
    try {
